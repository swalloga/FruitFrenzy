...
export default class Board {
  ...
  updateTile(r, c) {
    this.grid[r][c].clear();
    setTimeout(this.grid[r][c].drawTile(), 2400);
  }

  incrementTile(r, c) {
    this.grid[r][c].clearTemp();
    let newR = r;
    for (let i = 0; i < 200; i++) {
      newR += .005;
      setTimeout(this.grid[r][c].drawTemp(newR, c), 12);
    }
  }

  colorTileBackground(r, c) {
    this.grid[r][c].drawTileWithBackground();
  }
